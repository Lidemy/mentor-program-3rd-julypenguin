/* eslint-env jquery */
const url = 'http://julypenguin.tw/week21/index.php/api';
...
function registerbox() {
  const registerboxDiv = `
    <div class="register-wrapper">
      ...
      <form class="userBox" action="index.php/api/register" method="post" accept-charset="utf-8">
...
function loginbox() {
  const loginboxDiv = `
    <div class="login-wrapper">
      ...
      <form class="userBox" action="index.php/api/login" method="post" accept-charset="utf-8">
...
async function navbar() {
  ...
  $('.nav').append(userInfo.map(user => `
    ...
    <h4><a href='/week21/index.php' class='nav-link'>首頁</a></h4>
    ${/super_admin/.test(user.classification)
    ? "<h4><a href='/week21/index.php/authorization' class='nav-link'>權限管理</a></h4>" : ''}
    ${/admin/.test(user.classification)
    ? "<h4><a href='/week21/index.php/admin' class='nav-link'>後台管理</a></h4>" : ''}
    <h4><a href='/week21/index.php/api/logout' class='nav-link'>登出</a></h4>
  `));
}
...
async function notAdminLeave() {
  ...
  window.location = 'http://julypenguin.tw/week21/index.php';
}
...
function deleteComment() {
  ...
  setData('DELETE', 'destroy', id).then((res) => {
    if (res !== '1') {
      alert('刪除失敗');
    }
  });
...
function inputComment(e) {
  ...
  setData('POST', 'create', '', addContentData).then(res => console.log(res));
  $('.comment').val('');
}
...
function editOk(e) {
  ...
  setData('PATCH', 'update', id, updateData).then(res => alert(res === '1' ? '修改成功' : '修改失敗'));
  editCancel(e);
}
...
function editAuth(e) {
  if ($(e.target).hasClass('auth-submit')) {
    ...
    setData('PATCH', 'update/authorization', '', authData).then(res => alert(res === '1' ? '修改成功' : '修改失敗'));
  }
}
